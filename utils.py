...
# -----------------------------------------------------------------------------
# Data loading utilities for federated clients
# -----------------------------------------------------------------------------

def load_trainset():
    """
    Load the local training partition for this client.
    Expects an environment variable PARTITION_ID to be set (0, 1, 2, ...).
    """
    # Read partition index from environment
    partition_id = int(os.getenv("PARTITION_ID", "0"))
    # Path to the pre-saved partition
    data_dir = Path(__file__).parent / "client_data"
    file_name = f"part{partition_id}.pt"
    part_path = data_dir / file_name
    if not part_path.exists():
        raise FileNotFoundError(f"Training partition file not found: {part_path}")
    # Load and return
...
def load_testset():
    """
    Load the shared test set for evaluation.
    """
    data_dir = Path(__file__).parent / "client_data"
    file_name = "testset.pt"
    test_path = data_dir / file_name
    if not test_path.exists():
        raise FileNotFoundError(f"Testset file not found: {test_path}")
